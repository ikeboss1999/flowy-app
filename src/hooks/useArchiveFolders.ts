...
export function useArchiveFolders() {
    ...
    const key = activeUserId ? '/api/archive-folders' : null;
    const { data: folders = [], isLoading, mutate } = useSWR<ArchiveFolder[]>(key, fetcher);

    const addFolder = async (name: string): Promise<ArchiveFolder> => {
