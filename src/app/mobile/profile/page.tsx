...
export default function MobileProfile() {
    ...
    const handleSave = async () => {
        ...
        if (Object.keys(personalChanges).length > 0) {
            pendingChanges.personalData = { ...currentEmployee.personalData, ...personalChanges };
        }

        if (formData.iban !== currentEmployee.bankDetails.iban) {
            pendingChanges.bankDetails = { ...currentEmployee.bankDetails, iban: formData.iban };
        }
                ...
                <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-indigo-900/10 relative overflow-hidden group">
                    ...
                    <div className="relative z-10 flex flex-col gap-6">
                        ...
                        <div className="space-y-3">
                            ...
                            {(currentEmployee.employment as any)?.department && (
                                <div className="flex items-center gap-4">
                                    <Briefcase className="h-4 w-4 text-white/30 shrink-0" />
                                    <p className="text-sm font-medium text-slate-300">Abt: {(currentEmployee.employment as any)?.department}</p>
                                </div>
                            )}
                            {currentEmployee.employment?.startDate && (
                                <div className="flex items-center gap-4">
                                    <Calendar className="h-4 w-4 text-white/30 shrink-0" />
                                    <p className="text-sm font-medium text-slate-300">Eintritt: {new Date(currentEmployee.employment.startDate).toLocaleDateString("de-AT")}</p>
                                </div>
                            )}
                        </div>
