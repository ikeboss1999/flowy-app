...
export default function SettingsPage() {
    ...
    return (
        <div className="dashboard-page-centered">
            <div className="max-w-5xl mx-auto space-y-10 lg:space-y-16">
                {/* Header */}
                <div className="flex flex-col items-center text-center gap-4">
                    ...
                    <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900 tracking-tight font-outfit">Einstellungen</h1>
                ...
                <div className="flex justify-center">
                    <div className="flex items-center p-2 bg-slate-100/80 backdrop-blur-md rounded-[24px] border border-slate-200/50 shadow-inner w-full max-w-full overflow-x-auto sm:w-fit">
                        {TABS.map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id)}
                                className={cn(
                                    "shrink-0 px-5 sm:px-7 lg:px-10 py-4 lg:py-5 rounded-[20px] font-black text-sm lg:text-base transition-all duration-500 relative overflow-hidden",
                                    activeTab === tab.id
                ...
                <div className="pt-4 max-w-4xl mx-auto w-full">
                    ...
                    {activeTab === "Dokumente" && (
                        <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out space-y-8">
                            {/* Sub-tab strip */}
                            <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl p-1.5 w-full max-w-full overflow-x-auto sm:w-fit">
                                {DOC_SUBTABS.map(({ id, label, icon: Icon }) => (
                                    <button
                                        key={id}
                                        onClick={() => setDocSubTab(id)}
                                        className={cn(
                                            "flex shrink-0 items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all",
                                            docSubTab === id
