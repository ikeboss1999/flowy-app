...
import { Ban, CheckCircle2, ExternalLink, Search, ShieldAlert, Trash2 } from 'lucide-react';
...
export default function UserManagement() {
    ...
    const deleteAccount = (user: UserRow) => showConfirm({ title: 'Konto vollständig löschen?', message: `${user.companyName || user.name} ist gesperrt. Alle Daten, Storage-Dateien und Benutzer werden unwiderruflich aus dem laufenden System entfernt und vorher als Backup gespeichert.`, confirmLabel: 'Backup erstellen & löschen', cancelLabel: 'Abbrechen', variant: 'danger', onConfirm: async () => { const response = await fetch(`/api/admin/users?userId=${encodeURIComponent(user.companyOwnerId)}`, { method: 'DELETE' }); const payload = await response.json().catch(() => ({})); if (!response.ok) return showToast(payload.message || 'Löschung fehlgeschlagen.', 'error'); showToast('Konto wurde gesichert und vollständig gelöscht.', 'success'); load(); } });
    ...
    return <div className="mx-auto max-w-[1500px] space-y-7 p-6 lg:p-10">
        ...
        {filter === 'suspended' && rows.filter(user => user.id === user.companyOwnerId && user.access?.is_suspended).length > 0 && <section className="rounded-3xl border border-rose-200 bg-rose-50/60 p-5"><div className="mb-3 flex items-center gap-2 text-rose-800"><Trash2 className="h-4 w-4" /><h2 className="text-sm font-black">Gesperrte Konten löschen</h2></div><div className="flex flex-wrap gap-3">{rows.filter(user => user.id === user.companyOwnerId && user.access?.is_suspended).map(user => <button key={user.id} onClick={() => deleteAccount(user)} className="flex items-center gap-2 rounded-xl bg-rose-600 px-4 py-2.5 text-xs font-black text-white"><Trash2 className="h-3.5 w-3.5" /> {user.companyName || user.name} löschen</button>)}</div></section>}
