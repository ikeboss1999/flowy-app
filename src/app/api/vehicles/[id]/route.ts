...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
    ...
    try {
        if (isWeb) {
            ...
            writeLog('VehicleAPI', `Direct delete successful for ID: ${id}`);

            // Silent Sync
            const client = UnifiedDB.getAuthenticatedClient(session);
            client.from('vehicles').delete().eq('id', id).eq('userId', userId).then(({ error }) => {
                if (error) {
                    writeLog('VehicleAPI', `Direct cloud delete failed for ID: ${id}. Error: ${error.message}`);
                } else {
                    writeLog('VehicleAPI', `Direct cloud delete successful for ID: [${id}]`);
                }
            });
        }
