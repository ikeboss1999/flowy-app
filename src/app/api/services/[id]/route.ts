...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
    ...
    try {
        if (isWeb) {
            ...
            writeLog('ServiceAPI', `Direct delete successful for ID: ${id}`);

            // Silent Sync
            const client = UnifiedDB.getAuthenticatedClient(session);
            client.from('services').delete().eq('id', id).eq('userId', userId).then(({ error }) => {
                if (error) {
                    writeLog('ServiceAPI', `Direct cloud delete failed for ID: ${id}. Error: ${error.message}`);
                } else {
                    writeLog('ServiceAPI', `Direct cloud delete successful for ID: ${id}`);
                }
            });
        }
