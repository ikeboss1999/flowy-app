...
export function OfferPreviewModal({ isOpen, onClose, offer, customer, companySettings }: OfferPreviewModalProps) {
    ...
    const handleCreateInvoice = async () => {
        ...
        try {
            ...
            const defaultPaymentTerm = invoiceSettings.paymentTerms.find(pt => pt.id === invoiceSettings.defaultPaymentTermId) || invoiceSettings.paymentTerms[0];
            ...
            const newInvoice = {
                ...
                projectId: offer.projectId || "",
                notes: offer.notes ? `${offer.notes}\n\nBezug auf Angebot: ${offer.offerNumber}` : `Bezug auf Angebot: ${offer.offerNumber}`,
                createdAt: new Date().toISOString(),
                updatedAt: new Date().toISOString(),
            };
