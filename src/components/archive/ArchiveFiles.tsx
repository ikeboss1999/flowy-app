...
interface ArchiveFilesProps {
    title?: string;
}

export function ArchiveFiles({ title = "Allgemeines Archiv" }: ArchiveFilesProps) {
    const { files, isLoading: isLoadingFiles, uploadFile, deleteFile, getSignedUrl, updateFile } = useArchiveFiles();
    const { folders, isLoading: isLoadingFolders, addFolder, renameFolder, deleteFolder } = useArchiveFolders();
    ...
    const [movingFile, setMovingFile] = useState<ArchiveFile | null>(null);
    ...
    const resolveAndOpen = async (file: ArchiveFile) => {
        ...
        <div className="space-y-5">
            <div className="flex items-center gap-3">
                ...
                <div className="ml-auto flex gap-2">
                    ...
                    <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 shadow-md shadow-indigo-250 disabled:opacity-60">
