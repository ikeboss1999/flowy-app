...
const inputClasses = "w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5 font-bold text-slate-800 outline-none transition-all placeholder:text-slate-400 focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10";
...
export function ServiceModal({ isOpen, onClose, onSave, initialService, folders, defaultFolder, mode }: ServiceModalProps) {
    ...
    useEffect(() => {
        if (initialService) {
            ...
            setFormData({
                ...
                itemType: "standard",
                folder: isPositionMode ? defaultFolder || folders?.[0] || null : null
            });
        }
    }, [initialService, isOpen, isPositionMode, defaultFolder, folders]);
    ...
    return (
        <div className="fixed inset-0 z-[160] flex items-center justify-center bg-white/30 p-4 animate-in fade-in duration-200">
            <div className="flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-[36px] border border-white/20 bg-white shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <form onSubmit={handleSubmit} className="grid flex-1 overflow-y-auto bg-slate-50/60 xl:grid-cols-[310px_1fr]">
                    <aside className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-indigo-950 to-violet-900 p-5 text-white sm:p-6">
                        <div className="absolute right-0 top-0 h-48 w-48 rounded-full bg-fuchsia-500/20 blur-3xl" />
                        <div className="relative space-y-4">
                            <div className="rounded-[28px] border border-white/10 bg-white/10 p-5 backdrop-blur">
                                <div className={cn("mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-white", previewMeta.color)}>
                            ...
                            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
                                ...
                                {isPositionMode && (
                                    <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur sm:col-span-2 xl:col-span-1">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-white/45">Ordner</p>
                                        <p className="mt-1 truncate text-sm font-black text-white">{formData.folder || "Ordner waehlen"}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <main className="space-y-5 p-6 sm:p-8">
                        ...
                        <section className="rounded-[28px] border border-slate-100 bg-white p-6 shadow-sm">
                            ...
                            <div className={cn("grid gap-4", isPositionMode ? "lg:grid-cols-3" : "md:grid-cols-2")}>
                                <div className="space-y-2">
                        ...
                        <div className="sticky bottom-0 -mx-6 -mb-6 flex gap-3 border-t border-slate-100 bg-white/90 p-6 backdrop-blur sm:-mx-8 sm:-mb-8 sm:px-8">
                            <button
                                type="button"
                                onClick={onClose}
                                className="flex-1 rounded-xl border border-slate-200 bg-white px-6 py-3.5 font-black text-slate-600 shadow-sm transition-colors hover:bg-slate-50"
                            >
                                Abbrechen
                            </button>
                            <button
                                type="submit"
                                className="bg-primary-gradient flex-[2] rounded-xl px-6 py-3.5 font-black text-white shadow-lg shadow-indigo-500/20 transition-all hover:-translate-y-0.5 active:scale-95"
                            >
